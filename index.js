...
app.get('/datashop', limiter, (req, res) => {
  ...
  conn.query('SELECT * FROM shop ORDER BY price DESC', function (error, results) {
    if (error) {
  ...
  conn.query('SELECT * FROM admin WHERE username = ? AND password = ?', [usernamehere, passwordhere], function (error, results) {
    if (error) {
      ...
      if (results.length == 1) {
        ...
        conn.query('INSERT INTO shop (name, url,price,ID) VALUES (?,?,?,?)', [namehere, urlhere,price,ID],  function (error, results) {
          if (error) {
  ...
  conn.query('SELECT * FROM admin WHERE username = ? AND password = ?', [usernamehere, passwordhere], function (error, results) {
    if (error) {
      ...
      if (results.length == 1) {
        ...
        conn.query('DELETE FROM shop WHERE ID= ?', [ID], function (error, results) {
          if (error) {
            console.log("usernamehere error 2" + namehere);
            console.log("passwordhere error 3" + urlhere);
            console.log("err error 3" + error);
...
io.on('connection', (socket, req) => {
  ...
  socket.on('chat message', (msg) => {
    ...
    if (msg.id == "1") {
      client.channels.cache.get("960861314585100328").send("<@&961292721983750194> มีคนสนใจสินค้า ชื่อเฟส:"+msg.name+" ip:"+ip)

    }
    else if (msg.id == "2") {
      client.channels.cache.get("960861351360757770").send("<@&961292721983750194> มีคนสนใจสินค้า ชื่อเฟส:"+msg.name+" ip:"+ip)

    }
    else if (msg.id == "3") {
     // client.channels.cache.get("960861351360757770").send("<@&961292721983750194>")

// ให้ส่งข้อมูลไปห้องที่ และส่งว่าอะไรในที่นี้จะเป็นการ แทค
...
client.login('OTYwOTAyMTc4NTM1NTM4Njk5.YkxL5A.MZCVccBNx0RhaBZK9ZwKrmDt-tg');
